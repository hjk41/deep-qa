...
def load_embedding(embeddingfile):
    ...
    Note that the vectors will be appended with two vectors:
        vectors[len(alphabet)]: vector for unknown words
        vectors[len(alphabet) + 1]: vector for empty words
    ...
    # append vectors for unknown and empty words
    v = [v for v in embedding.values()]
    embed_dim = v[0].shape[0]
    unknown_emb = numpy.random.rand(embed_dim)
    empty_emb = numpy.random.rand(embed_dim)
    v.append(unknown_emb)
    v.append(empty_emb)
    vectors = numpy.array(v) 
